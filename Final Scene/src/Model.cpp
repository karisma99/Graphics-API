...
Model::Model(ID3D11Device* dev, const char * const _modelName, Vect scale)
{
	mDevice = dev;
	pStdVerts = nullptr;
	pTriList = nullptr;
...
Model::Model(ID3D11Device* dev, Model::PreMadeModels pm)
{
	...
	case UnitBoxRepeatedTexture:
		...
		break;
	case UnitSkybox:
		ModelTools::CreateSkyBox(pStdVerts, numVerts, pTriList, numTris);
		break;
	case UnitPyramid:
...
void Model::privLoadDataFromFile(const char * const _modelName, StandardVertex*& pVerts, int& nverts, TriangleByIndex*& ptlist, int& ntri, Vect scale)
{
	// Read from file
	FileHandle fh;
	FileError  ferror;
	...
	for (int i = 0; i < nverts; i++)
	{
		tmp = &(pTmpVerts[i]);
		v = &(pVerts[i]);
		v->set(tmp->x * scale.X(), tmp->y * scale.Y(), tmp->z * scale.Z(), tmp->u, tmp->v, tmp->nx, tmp->ny, tmp->nz, tmp->r, tmp->g, tmp->b, (int) tmp->txt);
	}
