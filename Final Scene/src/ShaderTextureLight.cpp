...
void ShaderTextureLight::SetPointLightParameters(const Vect& pos, float r, const Vect& att, const Vect& amb, const Vect& dif, const Vect& sp, const int ind)
{
	PointLightData[ind].Light.Ambient = amb;
	PointLightData[ind].Light.Diffuse = dif;
	PointLightData[ind].Light.Specular = sp;
	PointLightData[ind].Position = pos;
	PointLightData[ind].Attenuation = att;
	PointLightData[ind].Range = r;
}

void ShaderTextureLight::SetSpotLightParameters(const Vect& pos, float r, const Vect& att, const Vect& dir, float spotExp, const Vect& amb, const Vect& dif, const Vect& sp, const int ind)
{
	SpotLightData[ind].Light.Ambient = amb;
	SpotLightData[ind].Light.Diffuse = dif;
	SpotLightData[ind].Light.Specular = sp;
	SpotLightData[ind].Position = pos;
	SpotLightData[ind].Direction = dir;
	SpotLightData[ind].Attenuation = att;
	SpotLightData[ind].Range = r;
	SpotLightData[ind].SpotExp = spotExp;
}
...
void ShaderTextureLight::SendLightParameters(const Vect& eyepos)
{
	...
	for (int i = 0; i < dl.numP; i++)
	{
		dl.PntLight[i] = PointLightData[i];
	}
	for (int i = 0; i < dl.numS; i++)
	{
...
void ShaderTextureLight::SendFogData(const Vect& color, const float& start, const float& range)
{
	Data_Fog df;
	df.FogColor = color;
	df.FogStart = start;
